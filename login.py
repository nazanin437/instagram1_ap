...
class LoginWindow:
    def __init__(self, parent):
        ...
        self.login_window = Toplevel(parent)
        self.login_window.title("Login")
        self.login_window.geometry("900x500")
        self.login_window.configure(bg="white")

        self.photo_login = PhotoImage(file=r"C:\Users\Ofogh Rayaneh0211\Desktop\my_p\user.png")
        photo_l = Label(self.login_window, image=self.photo_login, border=0)
        photo_l.place(x=400, y=20)

        self.username_label = Label(self.login_window, text="Username:", font=("Helvetica", 12), bg="white")
        self.username_label.place(x=300, y=150)
        self.username_entry = Entry(self.login_window, width=30, font=("Helvetica", 12))
        self.username_entry.place(x=400, y=150)

        self.password_label = Label(self.login_window, text="Password:", font=("Helvetica", 12), bg="white")
        self.password_label.place(x=300, y=200)
        self.password_entry = Entry(self.login_window, width=30, font=("Helvetica", 12), show="*")
        self.password_entry.place(x=400, y=200)

        self.login_button = Button(self.login_window, text="Login", font=("Helvetica", 12), bg="#62449d", fg="white", command=self.validate_login)
        self.login_button.place(x=400, y=250)

        self.result_label = Label(self.login_window, text="", font=("Helvetica", 12), bg="white")
        self.result_label.place(x=400, y=300)
        ...
        try:
            with open(self.file_path, "r") as file:
                for line in file:
                    parts = line.split(",")
                    if len(parts) >= 3:
                        stored_email, stored_username, stored_password = parts[:3]
                        if stored_username == username:
                            if stored_password == password:
                                ...
                                return
                self.result_label.config(text="Username not found!", fg="red")
        except FileNotFoundError:
            self.result_label.config(text="No users registered yet!", fg="red")
#======================================================================================================================================
    def open_new_window(self):
        new_window = Toplevel(self.login_window)
    ...
    def open_search_window(self):
        search_window = Toplevel(self.login_window)
        ...
        def search_user():
            ...
            try:
                with open(self.file_path, "r") as file:
                    for line in file:
                            email, username, _ = line.split(",")
                            users_dict[username] = {"posts": random.randint(1, 50),"followers": random.randint(10, 1000),"following": random.randint(5, 500),"bio": "Dream big, stay curious, and keep growing."}
                if q in users_dict:
                    user_data = users_dict[q]
    ...
    def open_user_page_window(self):
        user_page_window = Toplevel(self.login_window)
    ...
    def get_user_posts(self):
        return [
            {"id": 1, "title": "Post 1", "content": "This is content for post 1"},
            {"id": 2, "title": "Post 2", "content": "This is content for post 2"}
        ]
    ...
    def view_post(self, post):
        ...
        if post["id"] == 2: 
            photo = PhotoImage(file=r"C:\Users\Ofogh Rayaneh0211\Desktop\my_p\son2.png") 
        elif post["id"] == 1: 
            photo = PhotoImage(file=r"C:\Users\Ofogh Rayaneh0211\Desktop\my_p\son1.png")  
    ...
    def open_home_window(self):
        self.home_window = Toplevel(self.login_window)
